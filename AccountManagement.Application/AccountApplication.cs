    ...
    public class AccountApplication : IAccountApplication {
        ...
        public OperationResult Register(RegisterAccount command) {
            ...
            var account = new Account(command.FullName, command.UserName, password, command.Mobile,
                command.RoleId.HasValue ? command.RoleId.Value : 0, picturePath);
        ...
        public OperationResult Edit(EditAccount command) {
            ...
            var path = "ProfilePictures";
            var picturePath = _fileUploader.Upload(command.ProfilePhoto, path);
            account.Edit(command.FullName, command.UserName, command.Mobile, command.RoleId.HasValue ? command.RoleId.Value : 0, picturePath);
            _accountRepository.SaveChanges();
            return operation.Succeeded();
        }
